...
def adjust(
    image: Image.Image, brightness=1.0, contrast=1.0, saturation=1.0, sharpness=1.0
) -> Image.Image:
...
def rotate(image: Image.Image, angle: float) -> Image.Image:
    return image.rotate(
        max(-360, min(360, angle)), expand=True, resample=Image.Resampling.BICUBIC
    )
...
def perspective(
    image: Image.Image, points: Iterable[Iterable[float]], output_size: tuple[int, int]
) -> Image.Image:
...
def remove_background(image: Image.Image) -> Image.Image:
    try:
        ...
        raise RuntimeError(
            "Instale rembg e onnxruntime para usar remove-background"
        ) from exc
    result = remove(image)
...
def main() -> None:
    ...
    parser.add_argument(
        "--points", help="JSON com quatro pontos: [[x,y],[x,y],[x,y],[x,y]]"
    )
    ...
    if args.operation == "upscale":
        ...
        save_image(
            adjust(
                image, args.brightness, args.contrast, args.saturation, args.sharpness
            ),
            args.output,
        )
        ...
        save_image(
            perspective(image, json.loads(args.points), (args.width, args.height)),
            args.output,
        )
        ...
        Path(args.output).write_text(
            json.dumps(ocr(image), ensure_ascii=False, indent=2), encoding="utf-8"
        )
